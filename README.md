...
### 1. Exastro IT Automation Ver.2.7.1 の構築
SimpleModeは、Exastro IT Automation Ver.2.7.1にのみ対応しています。他のバージョンでの動作は検証していません。
以下の手順で、Exastro IT Automation Ver.2.7.1をインストールします。

1. Exastro IT Automation Ver.2.7.1のソースをダウンロードします。
   https://github.com/exastro-suite/exastro-docker-compose/archive/refs/tags/2.7.1.tar.gz
2. ダウンロードしたソースを任意のディレクトリに配置し、展開します。
   tar -xzvf exastro-docker-compose-2.7.1.tar.gz
3. ディレクトリ名を変更します。
   mv exastro-docker-compose-2.7.1 exastro-docker-compose
...
以降の手順は、[Exastro on Docker Compose - Online](https://ita-docs.exastro.org/ja/2.7/installation/online/exastro/docker_compose.html) のとおりです。
...
SimpleModeで異なるパラメータシート間でデータをコピーするためには、事前にMovementとコピー元パラメータシートの関連付けを行う必要があります。 本シートに登録することで、収集結果を格納するパラメータシートから構築用パラメータシートにデータをコピーする等の用途が実現できます。
...
| Flag      | 文字列(単一行) | 8192         | 収集初期値管理フラグ<br> 1: 収集初期値管理用データ |
...
| 項目                   | 値                                                           |
| ---------------------- | ------------------------------------------------------------ |
| ITAリンク              | Exastro IT AutomationのサービスURL<br/>例：http://ita.example.com:80|
