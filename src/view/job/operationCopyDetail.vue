<template>
  <el-dialog v-model="visible" :close-on-click-modal="false" :close-on-press-escape="false" class="detailBoxSheet"
    ...
    <div style="justify-content: space-between;display: flex;">
      ...
      <div style="justify-content: right;display: flex;">
        ...
        <div style="text-align: left;padding-right: 20px;">
          ...
          <!-- <el-tooltip
            effect="light"
            :content="loginOperation"
            placement="top"
          >
          <div class="">&nbsp;&nbsp;{{ loginOperation }}</div>
          </el-tooltip> -->
          <div>&nbsp;&nbsp;{{ loginOperation }}</div>
    ...
    <!-- <div style="text-align: center;padding: 0px 20px;">
        オペレーション名（基準日時)：{{ loginOperation }}
        <div style="text-align: right">{{ logOperationScheduledDate }}</div>
      </div>
    </div> -->
    ...
    <div v-show="isShow" style="margin-top: 5px;">
      <p>
        <span>　　パラメータシート：</span>
        <el-select v-model="paramsValue" placeholder="コピー元のパラメータシートを選択する" style="width: 410px;" :disabled="isBtnDisable" filterable @change="changeval">
          <el-option v-for="item in sheetRadios" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>&nbsp;&nbsp;&nbsp;&nbsp;
      </p>
      <span>　　オペレーション名：</span>
      <el-select v-model="operation" placeholder="コピー元のオペレーションを選択する" style="width: 410px" :disabled="isBtnDisable">
    ...
    <div  style="border: 1px solid #dcdfe6;margin-right: 19px;">
      <EditSheet v-if = "isShowditSheet" :dialogVisibleEdit="editvisible" @changeEdit="changeEdit" :movementName="movementName"
      :editParams="paraSheetLeftRegister" :editOperation="loginOperation" @setButtonStatus="setButtonStatus"
      @editSheetchangestatus="editSheetchangestatus"></EditSheet>
    </div>
...
import type { UploadProps, UploadRawFile, ElTooltip, UploadUserFile } from "element-plus";
...
import { importApi, pulldownApi } from "../../api/index";
import {
  Download,
  Upload,
  DArrowRight,
...
import { ElMessage, ElTable, ElSelect, ElCard, ElUpload, genFileId } from "element-plus";
...
export default defineComponent({
  ...
  props: ["dialogVisible", "type", "movementName", "parametersheet"],
  ...
  setup(props, { emit }) {
    ...
    const getMenuItem = (isParametersheetChange:boolean=false) => {
      ...
      //     NORMAL: 0,
      //   },
      //   movement: { LIST: [movementName.value] },
      // };
      // menuItem(obj).then((res: any) => {
    ...
    const getSheetList = (str: string,objval:string) => {
      try {
        ...
        // if (paramsSheetValue.value) {
        //   getPullDown(paramsSheetValue.value);
        // }
      } catch (error: any) {
        ElMessage({
          type: "error",
    ...
    watch(
      ...
      (val, old) => {
        ...
        // movementTableData.value = val.movementTableData;
        if (visible.value) {
    ...
    const getTableLeft = async (value: string) => {
      // loadingShow.value = true;
      // getPullDown(value);
      // let respullDown = await pulldownApi(value);
      // hostId = respullDown.data.data.host_name;
      ...
      //   },
      // };
      // optionName(params1, value).then((res: any) => {
      //   let data = res.data.data;
      //   // 件数
      //   tableDataLeft.length = 0;
      //   let host = store.getHost;
      ...
      //       for (const i in hostId) {
      //         if (hostId[i] == element.parameter.host_name) {
      //           if (i == el.managed_system_item_number) {
      //             result.push(element);
      //           } else if (i == el.hostgroup_id) {
      //             result.push(element);
      //           }
      //         }
      //       }
            ...
            optionName(params1, paraSheetLeft.value).then((res: any) => {
              ...
              data.forEach((baseObj: unknown) => {
                hostList.forEach(async (itemHost: any) => {
                  ...
                  // for (const i in hostId) {
                    // if (hostId[i] == obj1.parameter.host_name) {
                    //   if (i == itemHost) {
                    //     obj1.parameter.uuid = "";
                    //     obj1.parameter.operation_name_select = selectName;
                    //     if (obj1.parameter.file) {
                    ...
                    //     }
                    //     optionsArr.push(obj1);
                    //   } else if (i == itemHost.hostgroup_id) {
                    //     obj1.parameter.uuid = "";
                    //     obj1.parameter.operation_name_select = selectName;
                    //     if (obj1.parameter.file) {
                    //     } else {
              ...
              if (optionsArr.length == 0) {
                ...
                optionAllRegister(optionsArr, paraSheetLeftRegister.value)
                  .then((res: any) => {
                    ...
                    emit("changestatus", copystatus);
                    // getOperations(paramsSheetValue.value);
    ...
    const handleRemove: UploadProps["onRemove"] = (file, uploadFiles) => {
      // Object.assign(uploadFileCopy,uploadFiles[0])
      uploadFileCopy = {};
    };
    ...
    const importExcel = () => {
      ...
      // rowData.value = paramsSheetValue.value;
    ...
    const openEditSheet = () => {
      // editParams.value = paraSheetLeft.value
      // editOperation.value = loginOperation
